...
def main():
    ...
    out_dir = os.path.normpath(
        os.path.split(af.BA_burn_date_dir)[0] + os.sep + 'output') + '\\'  # Create and set output directory
    ...
    FMgoodQuality = af.extracting_good_quality_vals_from_FM_lut(
        FM_v6_QA_lut)  # print(FMqualityFiles)  # debugging output
    ...
    BAgoodQuality = af.extracting_good_quality_vals_from_BA_lut(
        v6_BAQA_lut)  # Retrieve list of possible QA values from the quality dataframe
    ...
    for i in range(0, len(FMFiles)):
        ...
        for county_masked in shapes:
            ...
            BAquality = None
            # --------------------------------------------------------------------------
            BAScaled = af.get_scaled_df(BABand, BAData,
                                        BAscaleFactor)  # Apply the scale factor using simple multiplication
            BA = None  # Close the GeoTIFF file
            FMScaled = af.get_scaled_df(FMBand, FMData,
                                        FMscaleFactor)  # Apply the scale factor using simple multiplication
            FM = None  # Close the GeoTIFF file

            # ----- MASK AND GET MEAN VALUE -----#
            BA_masked = np.ma.MaskedArray(BAScaled, np.in1d(BAqualityData, BAgoodQuality,
                                                            invert=True))  # Apply QA mask to the BA data
            FM_masked = np.ma.MaskedArray(FMScaled, np.in1d(FMqualityData, FMgoodQuality,
                                                            invert=True))  # Apply QA mask to the FM data

            BA_resampled = scipy.ndimage.zoom(BA_masked, 0.5,
                                              order=0)  # Resample by a factor of 2 with nearest neighbor interpolation
            ...
            FM_BA = np.ma.MaskedArray(FM_masked,
                                      np.in1d(BA_resampled, BAVal, invert=True))  # Mask array, include only BurnArea
        ...
        print(
            'index: {}  --- FileName: {}   ---- Time Elapsed: {}  seconds'.format(i, FMFiles[i], round(currentTime, 1)))
